...
bool ClassicSVfitInterface::IsInteresting (const reco::Candidate *l1, const reco::Candidate *l2)
{
  ...
  if (pType == kMuHad)
  {
    ...
    bool iso2 = true; //(userdatahelpers::getUserInt(l2,"byVLooseIsolationMVArun2v1DBoldDMwLT") == 1);
  ...
  else if (pType == kHadHad)
  {
    ...
    if (!iso1 || !iso2){
      if(!iso1)cout<<"ISO1"<<endl;
      if(!iso1)cout<<"ISO2"<<endl;
      //return false;
    } 
    */
    return true; // passed all requirements
